...
init_test_() ->
	{ok,{state,Value,LeaseTime,_StartTime},TimeLeft} = simple_cache_element:init([?VALUE,?LEASETIME]),
...
handle_cast_for_replace_test_() ->
	ExpectedState = {state,?VALUE,?LEASETIME,?STARTTIME},
	{noreply, {ok,Value}, State,TimeLeft} = simple_cache_element:handle_cast({replace,?VALUE},ExpectedState),
	[?_assertEqual(?VALUE,Value),
	 ?_assertEqual(ExpectedState,State),
	 ?_assertEqual(?TIMELEFT,TimeLeft)].
